<div class="container mx-auto px-4 py-6">
    ...
    <button
        ...
        @click="
            ...
            if (!template) {
                alert('Template subunit-template tidak ditemukan!');
                return;
            }

            $dispatch('open-modal-tab', {
                title: 'Add Unit',
                manual: '{{ route('subunits.store') }}',
                excel: '{{ route('subunits.import') }}',
                group: '{{ $units->id }}',
                ...
                <tbody id="activityTable">
                    ...
                    <tr class="text-center">
                        <td class="border py-2 px-4">
                            <input type="checkbox" name="selected[]" class="rowCheckbox" value="{{ $subunit->id }}">
                        </td>
                        <td class="border py-2 px-4">{{ $index + 1 }}</td>
                        <td class="border py-2 px-4">{{ $subunit->id }}</td>
                        <td class="border py-2 px-4 text-start">{{ $subunit->name }}</td>
                        <td class="border py-2 px-4">
                            <form action="{{ route('subunits.destroy', $subunit->id) }}" method="POST" onsubmit="return confirm('Are you sure want to delete this?')">
                                <a href="{{ route('subunits.edit', $subunit->id) }}" class="text-blue-600 hover:text-blue-800">Edit</a> |
                                @csrf
                                @method('DELETE')
