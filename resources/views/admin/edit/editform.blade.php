...
<div class="max-w-xl mx-auto mt-10 bg-white shadow-md rounded px-8 pt-6 pb-8">
  ...
  <form action="{{ route('forms.update', $form->id) }}" method="POST" class="space-y-4">
    ...
    <div class="mb-4">
      <label for="no_urut" class="block text-gray-700 font-bold mb-2">Nomor Urut:</label>
      <input type="number" min="1" name="no_urut" id="no_urut" value="{{ old('no_urut', $form->no_urut) }}"
    ...
    <div class="mb-4">
      ...
      <select name="formtype_id" required class="validate-required activity-dropdown border rounded px-4 py-2 w-full">
        <option value="">Choose Activity</option>
           @foreach($formtypes as $type)
                <option value="{{ $type->id }}"
                    {{ $type->id == $form->formtype_id ? 'selected' : '' }}>
    ...
    <div class="flex justify-between items-center">
       <a href="{{ route('admin.units', $form->group_id) }}?tab=questions"  
