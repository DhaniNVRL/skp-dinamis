            ...
            @foreach ($visibleQuestions as $question)
                @php
                    ...
                    $questionSubunits = $subunits
                        ->filter(
                            fn ($item) => $activeSubunitIds->contains((int) $item->id)
                        )
                        ->values();
                @endphp
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs text-gray-500">
                                    ...
                                    @elseif ($questionTypeId === 7)
                                        Penilaian dengan alasan ketika Kinerja lebih dari 0
                                    @elseif ($questionTypeId === 8)
                                        Alasan ketika Kinerja lebih rendah daripada Kepentingan
                                    @endif
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-5 bg-gray-50 p-5">
                        @foreach ($questionSubunits as $subunit)
                            ...
                            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
                                ...
                                <div
                                    class="p-4 md:p-5"
                                    @if (in_array($questionTypeId, [7, 8], true))
                                        x-data="{
                                            importance: null,
                                            performance: null,
                                            ...
                                            get showReason() {
                                                ...
                                                return this.importance !== null
                                                    && this.importance > 0
                                                    && this.performance < this.importance;
                                            }
                                        }"
                                        x-on:change="
                                            if ($event.target.matches('input[type=radio]')) {
                                                if ($event.target.name === 'importance_{{ $questionScope }}') {
                                                    importance = Number($event.target.value);
                                                }

                                                if ($event.target.name === 'performance_{{ $questionScope }}') {
                                    ...
                                    @endif
                                >
                                    @if (in_array($questionTypeId, [2, 3, 4, 7, 8], true))
                                        @include(
                                        ...
                                        @elseif ($questionTypeId === 4)
                                            <div class="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4">
                                                <h5 class="mb-3 text-sm font-semibold text-gray-800">
                                                    Pilihan Alasan
                                                </h5>
                                        ...
                                        @elseif (in_array($questionTypeId, [7, 8], true))
                                            <div
                                                x-cloak
                                                x-show="showReason"
