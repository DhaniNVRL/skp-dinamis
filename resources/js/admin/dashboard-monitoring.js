...
const MonitoringDashboard = {
    ...
    refreshGroupOptions(resetInvalid) {
        if (!this.activitySelect || !this.groupSelect) {
            return;
        }

        const activityId = this.activitySelect.value;

        [...this.groupSelect.options].forEach((option, index) => {
            if (index === 0) return;
        ...
        if (resetInvalid && this.groupSelect.selectedOptions[0]?.disabled) {
            this.groupSelect.value = '';
        }
    },

    refreshUnitOptions(resetInvalid) {
        ...
        [...this.unitSelect.options].forEach((option, index) => {
            if (index === 0) return;
            const visible = !groupId || option.dataset.groupId === groupId;
            option.hidden = !visible;
            option.disabled = !visible;
        });

        if (resetInvalid && this.unitSelect.selectedOptions[0]?.disabled) {
            this.unitSelect.value = '';
        }
    },
    ...
    bindDetailButtons() {
        document.addEventListener('click', (event) => {
            ...
            this.openRespondentDetail(button.dataset.url);
        });
    },
    ...
    async openRespondentDetail(url) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Data responden tidak dapat dimuat.');
            }
            ...
            loading?.classList.add('hidden');
            if (error) {
                error.textContent = exception.message || 'Terjadi kesalahan saat mengambil data.';
    ...
    renderProfile(profile) {
        const grid = document.getElementById('respondentProfileGrid');
        if (!grid) return;
    ...
    renderSurvey(survey) {
        const summary = document.getElementById('respondentSurveySummary');
        if (!summary) return;
        ...
        status.className = `inline-flex rounded-full px-3 py-1 text-xs font-semibold ${statusColor}`;
        status.textContent = survey.status_label;
    ...
    renderAnswers(answers) {
        ...
        if (!answers.length) {
            ...
            cell.colSpan = 4;
            cell.className = 'px-4 py-10 text-center text-sm text-gray-500';
            cell.textContent = 'Responden belum mempunyai jawaban.';
        ...
        answers.forEach((answer, index) => {
            ...
            const numberCell = this.createCell(index + 1, 'text-gray-500');
            const formCell = this.createCell(answer.form || '-', 'font-medium text-gray-700');
            const questionCell = this.createCell(
                `${answer.question_no ? `${answer.question_no} - ` : ''}${answer.question || '-'}`,
                'text-gray-700'
            );
            const answerCell = this.createCell(this.formatAnswer(answer.answer), 'whitespace-pre-wrap text-gray-700');

            if (answer.competitor || answer.subunit) {
                const meta = document.createElement('p');
                meta.className = 'mt-1 text-xs text-gray-400';
                meta.textContent = [answer.competitor, answer.subunit].filter(Boolean).join(' • ');
                answerCell.appendChild(meta);
            }

            row.append(numberCell, formCell, questionCell, answerCell);
            body.appendChild(row);
        });
    },
    ...
    createCell(value, extraClass = '') {
        const cell = document.createElement('td');
        cell.className = `px-4 py-3 text-sm ${extraClass}`;
    ...
    formatAnswer(value) {
        ...
        return Object.entries(value)
            .map(([key, item]) => `${this.humanize(key)}: ${this.formatAnswer(item)}`)
            .join('\n');
    },
