...
document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    if (activity && group && unit) {
        // Simpan semua opsi asli dari Blade sebelum dropdown diubah.
        ...
        // Simpan pilihan dari URL sebelum opsi dibangun ulang.
        const initialGroupId = String(group.value || '');
        const initialUnitId = String(unit.value || '');

        /*
        |--------------------------------------------------------------------------
        | Filter Group berdasarkan Activity ID
        |--------------------------------------------------------------------------
        */
        ...
        const refreshGroupOptions = (selectedGroupId = '') => {
            ...
            // Hapus seluruh opsi lama.
            group.replaceChildren(
                new Option('Semua Group', '')
            );

            // Tampilkan Group hanya jika Activity sudah dipilih.
            if (activityId !== '') {
                allGroups.forEach(option => {
                    const groupActivityId = String(
                    ...
                    if (groupActivityId === activityId) {
                        ...
                        // Hapus status selected bawaan dari HTML.
            ...
            // Pertahankan pilihan dari URL jika masih sesuai Activity.
            ...
            group.value = validGroup
                ? String(selectedGroupId)
                : '';

            // Group tidak dapat dipilih sebelum Activity dipilih.
            group.disabled = activityId === '';
        };

        /*
        |--------------------------------------------------------------------------
        | Filter Unit berdasarkan Group ID
        |--------------------------------------------------------------------------
        */
        ...
        const refreshUnitOptions = (selectedUnitId = '') => {
            ...
            // Tampilkan Unit hanya jika Group sudah dipilih.
            if (groupId !== '') {
            ...
            // Pertahankan pilihan dari URL jika masih sesuai Group.
            const validUnit = Array.from(unit.options).some(
                option =>
                    option.value !== '' &&
                    option.value === String(selectedUnitId)
            );

            unit.value = validUnit
                ? String(selectedUnitId)
                : '';

            // Unit tidak dapat dipilih sebelum Group dipilih.
        ...
        */
        ...
        activity.addEventListener('change', () => {
            // Reset Group dan Unit ketika Activity berubah.
            refreshGroupOptions();
            refreshUnitOptions();
        });
        ...
        */
        ...
        group.addEventListener('change', () => {
            // Reset Unit ketika Group berubah.
            refreshUnitOptions();
        });
        ...
        |--------------------------------------------------------------------------
        |
        | Pilihan Group dan Unit dari URL tetap dipertahankan
        | jika relasinya sesuai dengan Activity dan Group.
        |
        */
