    ...
    */
    ...
    function initializeCustomerAssessment(page) {
        page.querySelectorAll(
        ...
        page.addEventListener("change", function (event) {
            const input = event.target.closest(
                "[data-performance-input], [data-importance-input]"
            );
            ...
            // Jangan tampilkan error baru ketika pengguna
            // sedang memilih Kepentingan atau Kinerja.
            // Hanya hapus error yang sudah ada jika jawaban lengkap.
            ...
            if (
                hasVisibleError &&
                validateQuestionContainer(container).valid
            ) {
                clearQuestionValidation(container);
            }
        });
    }
    ...
    function refreshCustomerReason(container) {
        if (!container) {
            return;
        }

        const questionType = Number(container.dataset.questionType);

        if (![3, 4, 7, 8].includes(questionType)) {
            return;
        }

        const reasonContainer = container.querySelector(
        ...
        const checkedPerformance = container.querySelector(
            "[data-performance-input]:checked"
        );

        const checkedImportance = container.querySelector(
            "[data-importance-input]:checked"
        );

        const performanceValue = checkedPerformance
            ? Number(checkedPerformance.value)
            : null;

        const importanceValue = checkedImportance
            ? Number(checkedImportance.value)
            : null;
        ...
        // Tipe 3 dan 4: aturan lama tetap dipertahankan.
        ...
        // Tipe 7: Kinerja harus dipilih dan lebih dari 0.
        ...
        // Tipe 8: Kinerja lebih rendah daripada Kepentingan.
        ...
        const reasonInput = reasonContainer.querySelector(
            "[data-performance-reason-input]"
        );

        if (reasonInput) {
            reasonInput.disabled = !shouldShow;
            reasonInput.required = shouldShow;

            if (!shouldShow) {
                reasonInput.value = "";
                clearMeaningfulFieldValidation(reasonInput);
            }
        }

        const checkboxGroup = reasonContainer.querySelector(
        ...
        checkboxGroup.querySelectorAll(
            ...
            if (!shouldShow) {
                ...
                const targetId = checkbox.dataset.childTarget;
    ...
    */
    ...
    function refreshLiveValidation(field) {
        ...
        // Validasi jawaban bermakna yang sudah ada.
        if (field.matches("[data-meaningful-answer]")) {
            if (
                !String(field.value ?? "").trim() ||
                !isMeaninglessAnswer(field.value)
            ) {
                clearMeaningfulFieldValidation(field);
            }
        }

        // Sebelum pengguna mencoba menyimpan,
        // jangan tampilkan border atau pesan error.
        if (container.dataset.validationAttempted !== "1") {
            return;
        }

        const result = validateQuestionContainer(container);

        if (result.valid) {
            /*
            * Untuk feedback, duplicate akan diperiksa
            * oleh fungsi validasi duplicate yang sudah ada.
            */
            if (!field.matches("[data-feedback-field]")) {
                clearQuestionValidation(container);
            }
        ...
        // Error hanya ditampilkan setelah percobaan submit.
        markQuestionInvalid(
            container,
            result.message,
            result.field
