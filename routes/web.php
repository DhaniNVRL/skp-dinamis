...
Route::get(
    ...
    Route::middleware('role:surveyor')->group(function () {
        ...
        Route::get(
            '/surveyor/respondent/{userId}/profile',
            [MonitoringController::class, 'respondentProfile']
        )->whereNumber('userId')->name('surveyor.respondent.profile');

        Route::get(
            '/surveyor/respondent/{userId}/answers',
            [MonitoringController::class, 'respondentAnswers']
        )->whereNumber('userId')->name('surveyor.respondent.answers');
    });

    Route::middleware('role:monitoring')->group(function () {
...
Route::delete('/groups/{group}/branch-rules/{rule}', [SurveyBranchRuleController::class, 'destroy'])
        ...
        Route::middleware('role:user,surveyor')->group(function () {
            Route::get(
                '/user/dashboard',
            ...
            Route::controller(ProfileController::class)
                ...
                Route::post('/surveyor/reset-account', 'resetOwnAccount')
                    ->name('surveyor.reset-account');
...
Route::get('/', function () {
    ...
    return match ($role) {
        ...
        'surveyor' => redirect()->route('user.dashboard'),
